...
3. **コンテストを一括セットアップ** — 全問題のディレクトリ・サンプル・テンプレートと、問題メタ情報の `contest.json`(タイトル・問題 URL・提出 URL)をまとめて用意します。
...
| コマンド | 別名 | 引数 | 説明 |
|---|---|---|---|
| `atr new` | `atr n` | `[-s]` コンテスト ID(例 `abc300`) | 全問題のディレクトリ + サンプル + テンプレート + `contest.json` を一括作成。`-s` で対象の問題を TUI で選択 |
...
- `-b BUILD` 全ケースの前に 1 回だけ実行するビルドコマンド(既定は設定の `build`、なければなし)。失敗したらテストせず即終了
- `-t SECONDS` 時間制限秒(既定 10、`0` で無制限)
- `-d DIR` テストケースの置き場所(既定 `test`)

`-c` / `-b` の中の `{task}` は実行ディレクトリ名に置換されます(例: `abc300/a` で実行すると `a`)。cabal や cargo で「この問題のターゲット」を指すのに使えます。

## Judgement

**`atr t` は指定されたコマンドを実行し、標準出力をサンプルと比較します。**
C++ のようにビルドが要る言語は、先に自分でビルドするか(既定コマンド `./a.out`)、設定の `build` に書いておくと全ケースの前に 1 回だけビルドが走ります(失敗したら即終了、ケースは実行されません)。
...
build = "g++ -O2 -o a.out main.cpp"  # atr t が全ケースの前に 1 回だけ実行(任意)
...
文字列は標準 TOML どおり、ダブルクォート(`\"` などのエスケープあり)とシングルクォート(リテラル)の両方が使えます。
`command` / `build` の中の `{task}` は実行ディレクトリ名に置換されます。ビルドシステム系の言語はこれで「いまの問題」を指せます。

```toml
# Haskell (cabal) の例: 問題ごとの実行ターゲットを {task} で指す
build = "cabal build {task} -v0"
command = "cabal run {task} -v0"
```
